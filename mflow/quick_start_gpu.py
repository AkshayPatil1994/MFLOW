...
#
# Quick start demonstration of GPU-accelerated multi-fidelity GPR model
#
def quick_demo_gpu(device='auto'):
    """
        Quick demonstration of the GPU-accelerated multi-fidelity GPR model.
    ...
    """
    print("\n" + "="*70)
    print("QUICK START: Multi-Fidelity GPR (GPU-Accelerated)")
    print("="*70 + "\n")

    # Initialize GPU model with memory management
    print("Step 1: Initializing GPU model...")
    model = MultiFidelityGPR_GPU(
    ...
    for i, angle in enumerate(test_angles):
        ...
        im = ax.contourf(x_grid, y_grid, pred.T, levels=20,
                        vmin=0, vmax=0.6, cmap='viridis')
        ax.set_title(f'MF-GPR (GPU) - {angle}°', fontsize=10)
        ax.set_aspect('equal')
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    ...
    plt.suptitle('Multi-Fidelity GPR (GPU): Wind Field Predictions', fontsize=14, y=0.995)
    plt.tight_layout()
    ...
    print("\nNote: GPU acceleration provides 10-100x speedup for larger datasets")
    print("  For better accuracy, train on more data points")

    # Save the trained model
    print("\nStep 8: Saving model...")
    model.save_model('mf_gpr_model_gpu.pth')
    print("   Model saved to 'mf_gpr_model_gpu.pth'")
